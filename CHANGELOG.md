...
- The column: `tenx` draws the list beside the session it embeds, in one process that owns the terminal, so the column's state survives switching tasks; `Ctrl+w` moves between the two.
