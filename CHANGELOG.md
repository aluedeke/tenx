...
- `tenx client` (experimental): the list as a column beside the tmux session in one process that owns the terminal — an embedded terminal on the right, so the column's state survives switching tasks.
