...
**Agent safety.** No function in the module writes a decrypted value to stdout, because an agent's captured stdout becomes a transcript that outlives the task. `decrypt` and `set` check whether `/dev/tty` is reachable, the same file `age`'s own prompt reads. When it is, they act. When it is not, which is the case inside an agent's shell tool, `decrypt` appends the request to a queue and returns, and `set` refuses outright, since a queued value would mean plaintext on disk before any human confirmed anything.
